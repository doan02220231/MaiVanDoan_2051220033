...
<div class="table-agile-info">
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      Chi tiết đơn hàng Pet cho thuê
    </div>
      ...
      <table class="table table-striped b-t b-light">
        ...
        <tbody>
            ...
            <td> {{$details->pet_qty * 7}} ngày </td>
             ...
             @endforeach
              @php
                    ...
                    $total = $totals;
              @endphp
          @foreach ($combo_pro as $key => $deta)
          @php
                                $comsup = $deta->combo_details_gia ?? 0; 
                                $combo += $comsup;
                                $total = $combo ? $totals + $combo : $totals;
                            @endphp

             <tr >
            <td>{{$deta->combo_details_name}}</td>        
            </td>
         
            <td>{{number_format($deta->combo_details_gia ,0,',','.')}} VNĐ</td>
          ...
          @endforeach
           ...
           <tr>
            <td style="font-weight: 1000; text-align: center;" >Vui lòng xử lí yêu cầu </td>
            <td colspan="12">
              @foreach($pet_show as $key => $or)
                @if($or->pet_status==0)
                <form>
                   @csrf
                  <select class="form-control pet_thue_trang_thai" id="{{$or->pet_id}}">
                    <option selected value="0">Chưa xử lý-chưa có chủ</option>
                    <option value="1">Pet đã duyệt-có chủ</option>
                  </select>
                </form>
                @else
                <form>
                  ...
                  <select class="form-control pet_thue_trang_thai" id="{{$or->pet_id}}">
                   <option  value="0">Chưa xử lý-chưa có chủ</option>
                    <option selected value="1">Pet duyệt đã có chủ</option>
                 </select>
                </form>
                @endif
                @endforeach
          ...
          <tr>
            <td style="font-weight: 1000; text-align: center;" > Phản hồi từ khách hàng khi nuôi pet </td>
            <td colspan="12">
              ...
              @foreach($order as $key => $mua)
                ...
                <form>
                   @csrf
                  <select class="form-control pet_thue_mua_ban" idd="{{$mua->order_thue_id}}" disabled>
          ...
          <tr>
               <td style="font-weight: 1000; text-align: center;" >Phản hồi từ khách hàng tình trạng sức khoẻ pet  </td>
               <td colspan="12">
              ...
              @foreach($order as $key => $khoe)
                ...
                <form>
                   @csrf
                  <select class="form-control pet_thue_suc_khoe" idd_s="{{$khoe->order_thue_id}}" disabled>
